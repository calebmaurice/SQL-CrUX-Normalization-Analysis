CREATE OR REPLACE TABLE calebmaurice.CrUX.retailer_web_vitals
  AS
  SELECT
    origin,
    date,
...
CREATE OR REPLACE TABLE `calebmaurice.CrUX.retailer_web_vitals_normalizedDevice`
AS
SELECT
  origin,
  date,
  ...
  phoneDensity,
  tabletDensity
FROM `calebmaurice`.`CrUX`.`retailer_web_vitals`
WHERE device IN ('desktop', 'phone', 'tablet');

  -- CREATE OR REPLACE TABLE `calebmaurice.CrUX.retailer_web_vitals_normalizedDevice_analysis`
  -- AS
  -- SELECT
  --   brand_name,
  --   AVG(p75_fcp) AS avg_p75_fcp,
  --   MAX(CASE WHEN device = 'desktop' THEN p75_fcp END) AS desktop_fcp,
  --   MAX(CASE WHEN device = 'phone' THEN p75_fcp END) AS mobile_fcp,
  --   MAX(CASE WHEN device = 'tablet' THEN p75_fcp END) AS tablet_fcp,
  --   ROUND(MAX(CASE WHEN device = 'desktop' THEN p75_fcp END), 2)
  --     AS desktop_fcp_ms,
  --   ROUND(MAX(CASE WHEN device = 'phone' THEN p75_fcp END), 2) AS mobile_fcp_ms,
  --   ROUND(
  --     MAX(CASE WHEN device = 'phone' THEN p75_fcp END)
  --       - MAX(CASE WHEN device = 'desktop' THEN p75_fcp END),
  --     2)
  --     AS absolute_gap_ms,
  --   ROUND(
  --     (
